namespace MSSQL.DIARY.COMN.Constant
{
    public static partial class SqlQueryConstant
    {
        ...
        public static string GetFunctionDependencies =
            @"  declare @Table table ( [name] varchar(100), [type] varchar(1000), Updated varchar(1000), selected varchar(1000), column_name varchar(1000) )INSERT INTO @Table exec sp_depends @function_name select * from @Table ";

        public static readonly string UpdateFunctionExtendedProperty =
            @"EXEC sys.sp_updateextendedproperty @name=N'MS_Description', @value=@fun_value,@level0type=N'SCHEMA',@level0name=@Schema_Name,@level1type=N'FUNCTION',@level1name=@FunctionName";

        public static readonly string CreateFunctionExtendedProperty =
            @"EXEC sys.sp_addextendedproperty @name=N'MS_Description', @value=@fun_value,@level0type=N'SCHEMA',@level0name=@Schema_Name,@level1type=N'FUNCTION',@level1name=@FunctionName";
    }
}