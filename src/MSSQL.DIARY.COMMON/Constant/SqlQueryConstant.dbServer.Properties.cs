namespace MSSQL.DIARY.COMN.Constant
{
    public static partial class SqlQueryConstant
    {
        ...
        public static readonly string GetTableColumns =
            @"SELECT  c.name AS column_name FROM sys.tables AS t INNER JOIN sys.columns c ON t.OBJECT_ID = c.OBJECT_ID WHERE t.name LIKE '%@tableName%' ";

        public static readonly string GetStoreProcedures =
            @"SELECT ((SCHEMA_NAME(O.SCHEMA_ID) )+'.'+ [NAME])AS 'STOREPROC' FROM SYS.SQL_MODULES M INNER JOIN SYS.OBJECTS O ON M.OBJECT_ID=O.OBJECT_ID
                                                              WHERE O.TYPE='P'";

        public static readonly string GetTigger =
            @"SELECT ((SCHEMA_NAME(O.SCHEMA_ID) )+'.'+ [NAME])AS 'Tiggers' FROM SYS.SQL_MODULES M INNER JOIN SYS.OBJECTS O ON M.OBJECT_ID=O.OBJECT_ID
                                                              WHERE O.TYPE='TR'";

        public static readonly string GetScalarFunctions =
            @"SELECT ((SCHEMA_NAME(O.SCHEMA_ID) )+'.'+ [NAME])AS 'SQL_SCALAR_FUNCTION' FROM SYS.SQL_MODULES M INNER JOIN SYS.OBJECTS O ON M.OBJECT_ID=O.OBJECT_ID
                                                              WHERE O.TYPE='FN'";

        public static readonly string GetTableValueFunctions =
            @"SELECT ((SCHEMA_NAME(O.SCHEMA_ID) )+'.'+ [NAME])AS 'SQL_TABLE_VALUED_FUNCTION' FROM SYS.SQL_MODULES M INNER JOIN SYS.OBJECTS O ON M.OBJECT_ID=O.OBJECT_ID
                                                              WHERE O.TYPE='TF'";

        public static readonly string GetAggregateFunctions =
            @"SELECT ((SCHEMA_NAME(O.SCHEMA_ID) )+'.'+ [NAME])AS 'SQL_AGGREGATE_FUNCTION' FROM SYS.SQL_MODULES M INNER JOIN SYS.OBJECTS O ON M.OBJECT_ID=O.OBJECT_ID
                                                              WHERE O.TYPE='IF'";

        public static readonly string GetUserDefinedDataType =
