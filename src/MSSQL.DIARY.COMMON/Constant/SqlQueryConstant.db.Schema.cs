namespace MSSQL.DIARY.COMN.Constant
{
    ...
    {
        public static string GetListOfAllSchemaAndMsDescription = @" SELECT sch.name , '' FROM sys.schemas sch where schema_id<=100 and [name] not in('sys','INFORMATION_SCHEMA','guest')";
        public static string GetSchemaInfoByName = @"";
        public static string GetAllSchemaReferancedObject = @"select name as table_name from sys.tables where schema_name(schema_id) = @schema_id order by name;";
